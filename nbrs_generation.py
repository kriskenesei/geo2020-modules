...
from shapely.geometry import (mapping, MultiPoint, LineString,
...
class nbrs_manager:
    ...
    def __init__(self, fpath):
        ...
        GeoDataFrame. The intended usage is as follows:
            ...
            5.  Optionally, plot either a single NBRS
                via .plot() or all NBRS via .plot_all()
                Optionally, write NBRS geometry to disc by
                invoking .write_all().
                [For NDW only: the buffered 3D line generation
                 feature is also best executed at this point via
                 the method .estimate_elevations_buffered().
                 The resulting 3D geometry can be written to disk
                 via invoking .write_buffered().]
            ...
            11. Active contour optimisation generates attractor
                ...
                Optionally, the optimised edges can also be
                written to disk using .write_contours, this is
                written to a single shapefile as usual.
            12. Invoke .build_tins() to construct the TIN surface
            ...
            13. Optionally, the TINs can be written to disk as
            ...
            14. Invoke .interpolate_elevations() to compute the
            ...
            15. Use .write_all() to write the 3D-NWB geometries
            ...
            16. Optionally, use generate_theoreticalerrors() to
                propagate input accuracy values to the output.
                These are stored in .wvk_z_errors.
            17. Optionally, use .write_origins() and .write_errors()
        ...
        An example execution is provided at the end of this script.
        """
    ...
    def write_errors(self, fpath):
        ...
        with open(fpath, "w") as file_out:
            ...
            for wvk_id, error in self.wvk_z_errors.items():
                ...
                file_out.write('{},{},{}\n'.format(wvk_id, dsy,
                                                   np.round(error, 3)))
    def write_buffered(self, fpath, ftype):
        """Writes the buffered 3D lines generated by method
        .estimate_elevations_buffered(), either as a Polygon or
        a LineString Shapefile. The argument "ftype" determines
        the geometry type. If it is set to "Polygon", then the
        entire set of buffered rings will be written as a single
        Polygon (with many interior rings). It can be set to
        "LineString" to write each 3D ring into the file as a
        separate LineString object.
        """
        if ftype not in ['Polygon', 'LineString']:
            print('Valid types: "Polygon", "LineString".'); return
        props = {
            'dist': 'float:15.2',
            'thres': 'float:15.2',
            'r': 'float:15.2'
        ...
        out_props = {
            'dist': self.buff_dist,
            'thres': self.buff_thres,
            'r': self.buff_r
            }
        if ftype == 'Polygon': geom = '3D Polygon'
        else:
            geom = '3D LineString'
            props['id'] = 'int'
        schema = {
        ...
        with fiona.open(fpath, 'w', 'ESRI Shapefile', schema) as file_out:
            if ftype == 'Polygon':
                file_out.write({
                    'geometry': mapping(Polygon(self.rings[0],
                                                self.rings[1:])),
                ...
                for i in range(len(self.rings)):
                    out_props['id'] = i
                    file_out.write({
                        'geometry': mapping(LineString(self.rings[i])),
                        'properties': out_props,
                        })
    ...
    def densify(self, thres):
        ...
        for wvk_id in self.wvk_geoms.keys():
            lstr = self.wvk_geoms[wvk_id]
            lstr, origins = lib_shared.densify_lstr(lstr, thres)
            self.set_wvk_geom(wvk_id, lstr)
    ...
    def estimate_elevations(self, fpath, r = 1, thin = 2):
        ...
        if not nbrs_ids:
            print('NBRS first need to be generated.'); return
    ...
    def estimate_elevations_buffered(self, dist, extrapolate = False,
                                     thres = 5, r = 1):
        """Method to compute the elevation of lines buffered
        from NWB geometry. Argument "dist" sets the distance
        by which NWB is buffered to obtain the 2D lines before
        computing their elevation.
        The method has two modes, it can either extrapolate
        from preliminary elevations or compute new elevations
        via AHN3. The former behaviour can be turned on by
        setting "extrapolate = True".
        The remaining arguments work this way:
        - "thres" sets the vertex densification threshold
          that is applied to the buffered centrelines before
          extrapolating or estimating their elevations
        - "r" sets the radius from which AHN3 points will
          be gathered to determine terrain elevation, if
          extrapolation is not being used
        The generated elevation series are smooth (they do
        not follow the terrain in an exact manner), and they
        generally approximate large AHN3 gaps in a linear
        manner, or at least effort has been made to avoid
        generating overshooting splines. Use .write_buffered()
        to write the resulting 3D geometry to disk.
        NOTE: this method depends on .estimate_elevations()
              which should thus first be run. However, this
              dependency is purely technical and could be
              avoided with further development.
        NOTE 2: This method was implemented only as a custom
                feature (requested by NDW). It is not part of
                my dissertation research.
        """
        if not self.flat_vxs:
            print('Elevations first need to be estimated.'); return
        print('\nSTARTING BUFFERED 3D LINE GENERATION')
        # buffer NWB geometry by a set amount
        # then, densify the resulting geometries and flatten
        # their coordinates into a single 1D array
        buff = MultiLineString(self.nwb.geometry.values).buffer(dist)
        buff_vxs = lib_shared.densify_lstr(LineString(buff.exterior),
                                           thres)[0].coords[:]
        lens = [len(buff_vxs)]
        for ring in buff.interiors:
            lstr = LineString(ring.coords[:])
            densified = lib_shared.densify_lstr(lstr, thres)[0]
            if densified:
                lens += [len(densified.coords[:])]
                buff_vxs += densified.coords[:]
        # if extrapolation is enabled, then associate each
        # vertex of the buffered geometries with the elevation
        # (preliminary or final) of the nearest NWB vertex
        if extrapolate:
            nbrs_tree = cKDTree(np.array(self.flat_vxs)[:,:2])
            _, ixs = nbrs_tree.query(buff_vxs)
            buff_vxs_3d = []
            for vx, ix in zip(buff_vxs, ixs):
                buff_vxs_3d.append((*vx[:2], self.flat_vxs[ix][2]))
            buff_vxs_3d = np.array(buff_vxs_3d)
        # if not, then gather AHN3 points (patches) close to
        # each vertex of the buffered geometries, and take their
        # median elevation as the elevation of each vertex
        else:  
            buff_tree = cKDTree(buff_vxs)
            lidar_tree = cKDTree(self.ahn3[:,1:3])
            lidar_ix = buff_tree.query_ball_tree(lidar_tree, r)
            buff_vxs_3d = np.c_[buff_vxs, np.zeros(len(buff_vxs))]
            i = 0
            for vx_ix in lidar_ix:
                if vx_ix: buff_vxs_3d[i, 2] = np.median(self.ahn3[vx_ix, 3])
                else: buff_vxs_3d[i, 2] = np.NaN
                i += 1
        self.rings, beg_ix = [], 0
        # refine the elevation in each buffered line (ring) separately
        for dlen in lens:
            ring = buff_vxs_3d[beg_ix:beg_ix + dlen]
            notnan = ~np.isnan(ring[:,2])
            if not extrapolate and len(ring[notnan]) > 10:
                # create a 2D profile from the ring and fill
                # missing elevations via linear interpolation
                diffs = np.insert(np.diff(ring[:,:2], axis = 0),
                                  0, 0, axis = 0)
                dsts = np.cumsum(np.sqrt((diffs ** 2).sum(axis = 1)))
                zs = ring[:,2].copy()
                zs[~notnan] = np.interp(dsts[~notnan], dsts[notnan],
                                        zs[notnan])
                # subsample the resulting rings of vertices
                # and fit continuous, periodic splines through them,
                # and derive the final elevations entirely from them
                samples = zs[::4]; samples[-1] = samples[0]
                model = CubicSpline(dsts[::4], samples, 0, 'natural')
                vals = model(dsts); vals[-1] = vals[0]
                ring[:,2] = vals
            self.rings.append(ring.round(5))
            self.buff_dist, self.buff_thres, self.buff_r = dist, thres, r
            beg_ix += dlen
        print('FINISHED BUFFERED 3D LINE GENERATION')
    ...
    def build_tin(self, max_dh_int, max_angle_int, r_int,
                  max_dh_ext, max_angle_ext, r_ext,
                  ext_steps, ext_dist, type_edges = 'optimised',
                  chosen = None):
...
if __name__ == '__main__':
    ...
    origins_fpath = '[PATH_TO_DIRECTORY]//C_39CZ1_origins.csv'
    errors_fpath = '[PATH_TO_DIRECTORY]//C_39CZ1_errors.csv'
    ...
    """ Note for NDW: to generate and export the buffered 3D
    lines, you need to run the following code.
    Please note that in the example below, a 25-metre buffering
    is used via AHN3-based elevation estimation. For extrapolation
    (and various other features), please see the docstrings of
    .estimate_elevations_buffered() and .write_buffered().
    This does NOT need to be run after executing all the above
    code, in fact (unless extrapolation is desired), the best
    place to run it is right after invoking .estimate_elevations().
    """
    """buffered_fpath = '[PATH_TO_DIRECTORY]//C_39CZ1_buffered_25.shp'
    roads.estimate_elevations_buffered(25, False)
    roads.write_buffered(buffered_fpath, 'LineString')"""